...
class SpikeMonitorCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        # Get layer outputs for monitoring
        layer_outputs = []
        for layer in self.model.layers:
            if hasattr(layer, 'output'):
                layer_outputs.append(layer.output)
        
        # Create a temporary model to get intermediate outputs
        monitor_model = Model(inputs=self.model.input, outputs=layer_outputs)
        outputs = monitor_model.predict(self.x_sample, verbose=0)
        
        # Log spike activity
        with self.file_writer.as_default():
            for i, (layer, output) in enumerate(zip(self.model.layers, outputs)):
                if 'conv' in layer.name or 'dense' in layer.name:
                    tf.summary.histogram(f"{layer.name}_spikes", output, step=epoch)
...
def parse_arguments():
    ...
    parser.add_argument('--data_name', type=str, default='CIFAR10', help='Dataset: MNIST | CIFAR10 | CIFAR100')
    parser.add_argument('--logging_dir', type=str, default='./logs/', help='Directory for logging')
    parser.add_argument('--model_type', type=str, default='SNN', help='Model type: SNN | ReLU')
...
# ==============================
# SIMPLIFIED SNN LAYERS (WORKING VERSION)
# ==============================
class SpikingDense(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        membrane_potential = tf.matmul(inputs, self.kernel) + self.bias
        # Simplified spiking behavior using ReLU
        spikes = tf.nn.relu(membrane_potential)
        return spikes

class SpikingConv2D(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size=(3,3), padding='same', name=None, **kwargs):
        super().__init__(name=name)
        self.filters = filters
        self.kernel_size = kernel_size
        self.padding = padding
    ...
    def call(self, inputs):
        # Convolutional membrane potential
        membrane_potential = tf.nn.conv2d(
            inputs, self.kernel, strides=[1, 1, 1, 1], padding=self.padding.upper()
        ) + self.bias
        # Simplified spiking behavior
        spikes = tf.nn.relu(membrane_potential)
        return spikes

# ==============================
# VGG16 SNN MODEL (FUNCTIONAL API)
# ==============================
def create_vgg16_snn(input_shape=(32, 32, 3), num_classes=10):
    """Create VGG16-style SNN using Functional API"""
    weight_decay = 0.0005
    inputs = Input(shape=input_shape)

    # Block 1
    x = SpikingConv2D(64, (3, 3), padding='same', name='conv1_1')(inputs)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.3)(x)
    
    x = SpikingConv2D(64, (3, 3), padding='same', name='conv1_2')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)

    # Block 2
    x = SpikingConv2D(128, (3, 3), padding='same', name='conv2_1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(128, (3, 3), padding='same', name='conv2_2')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)

    # Block 3
    x = SpikingConv2D(256, (3, 3), padding='same', name='conv3_1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(256, (3, 3), padding='same', name='conv3_2')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(256, (3, 3), padding='same', name='conv3_3')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)

    # Block 4
    x = SpikingConv2D(512, (3, 3), padding='same', name='conv4_1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(512, (3, 3), padding='same', name='conv4_2')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(512, (3, 3), padding='same', name='conv4_3')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)

    # Block 5
    x = SpikingConv2D(512, (3, 3), padding='same', name='conv5_1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(512, (3, 3), padding='same', name='conv5_2')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.4)(x)
    
    x = SpikingConv2D(512, (3, 3), padding='same', name='conv5_3')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    x = Dropout(0.5)(x)
    ...
    # Classifier
    x = Flatten()(x)
    x = SpikingDense(512, name='dense1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(0.5)(x)
    
    x = SpikingDense(num_classes, name='dense_out')(x)
    outputs = Activation('softmax')(x)

    model = Model(inputs=inputs, outputs=outputs, name='vgg16_snn')
    return model

# ==============================
# ANN VGG16 MODEL
# ==============================
def create_vgg16_ann(input_shape=(32, 32, 3), num_classes=10):
    """Create standard VGG16 ANN"""
    weight_decay = 0.0005
    inputs = Input(shape=input_shape)
    ...
    model = Model(inputs=inputs, outputs=outputs, name='vgg16_ann')
    return model
...
def create_model(args, data):
    ...
    if args.model_type == 'SNN':
        print("[INFO] Creating VGG16 SNN model...")
        model = create_vgg16_snn(data.input_shape, data.num_of_classes)
    else:
        print("[INFO] Creating VGG16 ANN model...")
        model = create_vgg16_ann(data.input_shape, data.num_of_classes)
...
def main():
    start_time = time.time()
    args = parse_arguments()
    args.model_name = args.data_name + "-VGG16-" + args.model_type

    # Setup logging
    set_up_logging(args.logging_dir, args.model_name)
    logging.info("### Starting VGG16 Training Script ###")
    ...
    data = Dataset(
        ...
        args.logging_dir,
        flatten=False,
        ttfs_convert=(args.model_type == 'SNN'),  # TTFS encoding for SNN
        ttfs_noise=args.noise,
    )
    ...
    if args.training:
        ...
        # Callbacks
        save_cb = SaveWeightsEveryNEpochs("weights/", n=10)
        ...
        checkpoint_cb = ModelCheckpoint(
            ...
            mode='max',
            save_weights_only=True  # Critical for Functional models
        )
        ...
        # Spike monitoring
        x_sample = data.x_train[:32]  # Small batch for monitoring
        spike_monitor_cb = SpikeMonitorCallback(log_dir=log_dir, x_sample=x_sample)

        callbacks = [save_cb, checkpoint_cb, tensorboard_cb, spike_monitor_cb]
        ...
        if args.save:
            model.save_weights("weights/final_model.weights.h5")
            logging.info("Model weights saved → weights/final_model.weights.h5")
