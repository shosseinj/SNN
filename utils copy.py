...
def copy_layer(orig_layer):
    ...
    #     return tf.keras.layers.InputLayer(input_shape=orig_layer.input_shape[1:], 
    #                                     dtype=orig_layer.input_dtype)

    if isinstance(orig_layer, tf.keras.layers.InputLayer):
    # First try the most reliable attribute in TF 2.x
        if hasattr(orig_layer, '_batch_input_shape'):
            input_shape = orig_layer._batch_input_shape[1:]  # Remove batch dimension
        # Fallback to other attributes
        elif hasattr(orig_layer, 'batch_shape') and orig_layer.batch_shape is not None:
            input_shape = orig_layer.batch_shape[1:]
        elif hasattr(orig_layer, 'input_spec') and orig_layer.input_spec is not None:
            input_shape = orig_layer.input_spec.shape[1:]
        else:
            raise AttributeError("Could not determine input shape for InputLayer")
        
        return tf.keras.layers.InputLayer(
            input_shape=input_shape,
            dtype=orig_layer.dtype,
    ...
    # Only call build if the layer isn't an InputLayer and has input_shape attribute
    if not isinstance(orig_layer, tf.keras.layers.InputLayer) and hasattr(orig_layer, 'input_shape'):
...
def fuse_bn(logging, model, p, q, optimizer, BN = True, BN_before_ReLU = False):
    """
    ...
    if BN:
        ...
        if BN_before_ReLU:
            ...
            # Batch normalization layers are always found after modelReLU activation function.
            while i<len(model.layers): 
