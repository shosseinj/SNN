...
class Conv2DWithBias(tf.keras.layers.Conv2D):
    ...
    def __init__(self, *args, **kwargs):
        kwargs['use_bias'] = False  # Disable standard bias
    ...
    def call(self, inputs):
        ...
        if self.use_custom_bias:
            # Static Python check (okay)
            padding_is_valid = (self.padding == 'valid')

            # Tensor boolean checks
            bn_not_1 = tf.not_equal(self.BN[0], 1.0)
            bn_before_relu_is_1 = tf.equal(self.BN_before_ReLU[0], 1.0)

            # Combine all with TensorFlow logic
            condition = tf.logical_or(
                padding_is_valid,
                tf.logical_or(bn_not_1, bn_before_relu_is_1)
            )

            def same_bias_case():
                return result + self.bias[0]

            def location_specific_bias_case():
                result_0 = result[:, 1:-1, 1:-1, :] + self.bias[0]
                result_1 = result[:, :1, :1, :] + self.bias[1]
                result_2 = result[:, :1, 1:-1, :] + self.bias[2]
                result_3 = result[:, :1, -1:, :] + self.bias[3]
                ...
                middle = tf.concat([result_8, result_0, result_4], axis=2)
                bottom_row = tf.concat([result_7, result_6, result_5], axis=2)
                return tf.concat([top_row, middle, bottom_row], axis=1)

            result = tf.cond(condition, same_bias_case, location_specific_bias_case)
...
def copy_layer(orig_layer):
    ...
    if isinstance(orig_layer, tf.keras.layers.InputLayer):
        # InputLayer uses 'batch_input_shape' in its config
        new_layer = tf.keras.layers.InputLayer.from_config(config)
        return new_layer
    
    input_shape = orig_layer.input.shape
   
    if hasattr(orig_layer, 'input_shape') and orig_layer.input_shape:
        input_shape = orig_layer.input_shape
    elif hasattr(orig_layer, 'batch_input_shape') and orig_layer.batch_input_shape:
        input_shape = orig_layer.batch_input_shape
    elif hasattr(orig_layer, '_batch_input_shape') and orig_layer._batch_input_shape:
        input_shape = orig_layer._batch_input_shape
...
def fuse_bn(data,model, p, q, optimizer, BN = True, BN_before_ReLU = False):
    """
...
# Explicitly build the model with input shape
    input_shape = model.input_shape[1:]  # (32, 32, 3)
    
    # Create new model with explicit input
    fused_model = tf.keras.Sequential(name="fused_model")
    fused_model.add(tf.keras.layers.InputLayer(input_shape=input_shape, name="input_layer"))
...
def fuse_imaginary_bn(fused_model, model, p, q): 
    ...
    if 'conv' in first_layer.name:
        ...
        # Sort to match exact weight order of the layer
        new_weights_sorted = [new_weights[i] for i in range(len(layer.weights))]

        layer.set_weights(new_weights_sorted)


        # layer.BN.assign([1])          # BN flag
        # layer.BN_before_ReLU.assign([0])
...
def fuse_bn_before_activation(fused_model, model, i):
    ...
    W_fused = tf.reshape(W_fused, tf.shape(previous_layer.kernel))    #[3,3,3,64]]
    # See Eq. 7.
...
def fuse_bn_after_activation(fused_model, model, i): 
    ...
    if 'flatten' in model.layers[i+1].name:
        ...
        fused_model.add(copy_layer(model.layers[i+1]))
        ft = model.layers[i+1]
        kappa = tf.tile(kappa, [ft.output.shape[-1]//ft.input.shape[-1]])
        b_term = tf.tile(tf.squeeze(b_term), [ft.output.shape[-1]//ft.input.shape[-1]])
        i+=1
