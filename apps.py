...
class MaxMinPool2D(tf.keras.layers.MaxPool2D):
    ...
    def build(self, input_shape):
        ...
        self.sign=tf.Variable(tf.constant(np.ones((1, 1, 1, input_shape[-1]),  dtype=np.float32)), dtype=tf.float32, name='sign', trainable=False)
    def call(self, inputs):
...
def fuse_bn_after_activation(fused_model, model, i): 
    ...
    while not hasattr(next_layer, 'kernel'):
        fused_model.add(copy_layer(next_layer))
        i += 1
        if i + 1 >= len(model.layers):
            raise ValueError("No layer with kernel found after BN layer.")
        next_layer = model.layers[i+1]
